...
@section('scripts')
    {{-- <script>
            ...
            var rectContainer = document.querySelector(".container").getBoundingClientRect();
            window.addEventListener('resize', initBoardsPositions);
            setTimeout(() => {
                header = document.querySelector(".container");
                headerIsExpended=true;
                group = document.querySelector(".d-flex");
                nodes = document.querySelectorAll(".col-lg-3");
                total = nodes.length;
                orderHelper =  Math.pow (10 , parseInt( String(total).length)) ;
                ease  = Power1.easeInOut;
                boxes = [];
                containerPadding = 12; //px
            }, 3000);
                ...
                for (var i = 0; i < total; i++) {
                    ...
                    if (!positionReplaced && numberOfElementsInRow > 1) {
                        ...
                        else
                        {
                            ...
                            // var distanceOfEdge;
                            if (moveUp) {
                                distance = parseInt(rect.right) - parseInt(rectContainer.left)
                                distanceSubstitutionalRect=parseInt(rectContainer.right) - parseInt(substitutionalRect.left);
                                distanceOFedgeSubstitutionalRect =parseInt(rectContainer.right) - parseInt(substitutionalRect.right);
                                delay = (1*distanceOFedgeSubstitutionalRect) / distanceSubstitutionalRect
                                TweenLite.fromTo(substitutional, 1, { x:0 },{ x: (distanceSubstitutionalRect) , display:'none',ease});
                                TweenLite.fromTo(box.node, 1, { x: -(distance) ,y:0}, { x: 0, y: 0 ,ease }).delay(delay);
                            }
                            else  {
                                distance = parseInt(rectContainer.right) - parseInt(rect.left)
                                distanceSubstitutionalRect=parseInt(substitutionalRect.right)-parseInt(rectContainer.left) ;
                                distanceOFedgeSubstitutionalRect =parseInt(substitutionalRect.left)-parseInt(rectContainer.left) ;
                                delay = (1*distanceOFedgeSubstitutionalRect) / distanceSubstitutionalRect
                                TweenLite.fromTo(substitutional, 1, { x:0},{ x:-(distanceSubstitutionalRect),display:'none',ease });
                                TweenLite.fromTo(box.node, 1, { x: (distance) ,y:0}, { x: 0, y: 0 ,ease }).delay(delay);
                            }
...
{{-- @section('scripts')
    <script>
            ...
            var boxes = [];
            var containerPadding = 12; //px
            var rect = document.querySelector(".boards-header").getBoundingClientRect();
        setTimeout(() => {
            header = document.querySelector(".container");
            headerIsExpended=true;
            group = document.querySelector(".d-flex");
